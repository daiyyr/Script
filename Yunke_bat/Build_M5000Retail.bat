...
rem M5000 project build script
rem Our software version format is like this:
rem ProductName_MajorVersion.MinorVersion.SVNVersion_DateNumber_ReleaseType

set ProductName=M5000
set MajorVersion=1
set MinorVersion=0
set ReleaseType=RETAIL

set TOP=Z:
set SVNPath=https://10.24.0.220:8443/svn/milk/trunk
...
copy partition.bin /b + qcsblhd_cfgdata.bin /b + qcsbl.bin /b + oemsbl.bin /b + amss.bin /b + appsboot.bin /b + splash.bin /b + boot.2knand.bin /b + system.2knand.bin /b + userdata.2knand.bin /b + recovery.2knand.bin  /b     F:\DailyBuilds\%ProductName%\%ROMVersionNumber%\%ROMVersionNumber%.bin /b
